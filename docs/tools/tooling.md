...
```
module github.com/guilhermegouw/matrix-cli  # Module path (import path)
...
- **Subcommand support:** Natural `matrix <command>` structure.
...
func newRootCmd() *cobra.Command {
    cmd := &cobra.Command{
        Use:   "matrix",           // How users invoke it
...
```bash
go build -ldflags "-X github.com/guilhermegouw/matrix-cli/cmd.Version=1.0.0"
```
...
| `task clean` | `rm -f matrix` | Remove build artifacts |
