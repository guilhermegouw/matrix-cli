...
// AuthorizeURL returns the Claude OAuth2 authorization URL.
...
func ExchangeToken(ctx context.Context, code, verifier string) (*oauth.Token, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("failed to exchange token: status %d body %q", resp.StatusCode, string(body))
	}
...
func RefreshToken(ctx context.Context, refreshToken string) (*oauth.Token, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("failed to refresh token: status %d body %q", resp.StatusCode, string(body))
	}
